...
**Single architecture:** the codebase has one model — the published **H LorentzNet
mass-shell flow**, with typed reference vectors, reference-contraction scalar features,
evolving auxiliary coordinates, and normalized log-map readout. `ModelConfig` is just
`{n_hidden, n_layers, regulator_mass}`.

**`src/` (code):** entrypoints stay at the root (so `python train.py` is unchanged);
library code is grouped by concern.
- `train.py` — single-process training entry.
...
- `models/` — `lorentznet_flow.py` (the published H model), `stage1/`
  (jet-attribute NF: `jet_attr_model_v3.py`).
...
- `configs/*.yaml` — final H training recipes and its inference defaults.
