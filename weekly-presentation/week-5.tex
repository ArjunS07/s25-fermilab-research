...
\begin{frame}[allowframebreaks]{New architecture}
    \begin{enumerate}
        ...
        \item Now, we use $L$ Lorentz-group equivariant layers. \begin{enumerate}
            ...
            \item Update the global embedding $g_{l+1} = \phi_{eg}(g^l, \frac{c}{N} \sum\limits_{i, j \in [N]} \phi_e(m_{ij}) , \sum\limits_{i, j \in [N]} \phi_e(m_{ij}))$
            \item Feed in the updated global embedding as well as the original embedding to update the graph embeddings $$
            x_i^{l+1} = x_i^{l} + \phi_x(g_0 , g^{l+1} , \frac{c}{N} \sum\limits_{i, j \in [N]} \phi_{ex}(m_{ij}) , \sum\limits_{i, j \in [N]} \phi_{ex}(m_{ij}))
            $$
        ...
        x_i^{L+1} = x_i^{L} + \phi_x(g_0 , g^{L} , \frac{c}{N} \sum\limits_{i, j \in [N]} \phi_{ex}(m_{ij}) , \sum\limits_{i, j \in [N]} \phi_{ex}(m_{ij}))
        $$
    \end{enumerate}
\end{frame}
...
\begin{frame}{Zero padding}
    ...
    \begin{itemize}
        ...
        \item Multiply results of $\phi_e(m_{ij})$, $\phi_{ex}(m_{ij})$ by expanded mask
