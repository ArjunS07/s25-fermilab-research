...
def minkowski_features(x, masks):
...
class LorentzFMNet(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor, zero_masks: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        for i, layer in enumerate(self.layers):
            if i == 0:
                vel = layer.forward(x_t, zero_masks, t)
            else:
                # Pass the output of the previous layer as input to the next layer
                vel = layer.forward(vel, zero_masks, t)
        return vel

    def step(self, x_t: torch.Tensor, zero_masks: torch.Tensor, t_start: torch.Tensor, t_end: torch.Tensor, method: ode_solver_methods=ode_solver_methods.euler) -> torch.Tensor:
        """
        ...
        if method == ode_solver_methods.euler:
            ...
            velocity = self.forward(x_t=x_t, zero_masks=zero_masks, t=t_start)
            print(f"{velocity.shape=}, {t_start.shape=}, {t_end.shape=}")
            print(f"Velocity: std={velocity.std()}, mean={velocity.mean()}")
            return x_t + (t_end - t_start) * velocity
            ...
            start_vel = self.forward(x_t=x_t, zero_masks=zero_masks, t=t_start)
            midpoint_x = x_t + (start_vel * (t_end - t_start) / 2)
            midpoint_vel = self.forward(x_t=midpoint_x, zero_masks=zero_masks, t=t_start + (t_end - t_start) / 2)
